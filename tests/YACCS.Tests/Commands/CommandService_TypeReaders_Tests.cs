...
public class CommandService_TypeReaders_Tests
{
	...
	public async Task ProcessTypeReaderMultipleButNotAllValues_Test()
	{
		...
		var result = await RunAsync<int[]>(4, input).ConfigureAwait(false);
	...
	public async Task ProcessTypeReaderMultipleValuesLongerThanArgs_Test()
	{
		var value = new[] { 1, 2, 3, 4 };
		var input = value.Select(x => x.ToString()).ToArray();
		var result = await RunAsync<int[]>(null, input).ConfigureAwait(false);
		Assert.IsTrue(result.InnerResult.IsSuccess);
	...
	public async Task ProcessTypeReaderNotRegistered_Test()
	{
		...
		{
			_ = await RunAsync<DBNull>(1, new[] { "joeba" }).ConfigureAwait(false);
	...
	{
		var result = await RunAsync<char>(1, new[] { "joeba" }, new CoolCharTypeReader()).ConfigureAwait(false);
		Assert.IsTrue(result.InnerResult.IsSuccess);
	}
	...
	{
		var result = await RunAsync<char>(1, new[] { "joeba" }).ConfigureAwait(false);
	...
	{
		const int VALUE = 2;
		var result = await RunAsync<int>(1, new[] { VALUE.ToString() }).ConfigureAwait(false);
		Assert.IsTrue(result.InnerResult.IsSuccess);
	...
	{
		const int VALUE = 2;
		var result = await RunAsync<int>(1, new[] { VALUE.ToString(), "joeba", "trash" }).ConfigureAwait(false);
		Assert.IsTrue(result.InnerResult.IsSuccess);
		Assert.IsInstanceOfType<int>(result.Value);
		Assert.AreEqual(VALUE, result.Value);
	}
	...
	{
		var result = await RunAsync<char[]>(4, new[] { "a", "b", "cee", "d" }).ConfigureAwait(false);
		Assert.IsFalse(result.InnerResult.IsSuccess);
	}
	...
	public async Task ProcessTypeReadersString_Test()
	{
		...
		var result = await RunAsync<string>(1, new[] { VALUE }).ConfigureAwait(false);
		Assert.IsTrue(result.InnerResult.IsSuccess);
	...
	{
		var result = await RunAsync<IContext>(0, new[] { "doesn't matter" }).ConfigureAwait(false);
		Assert.IsTrue(result.InnerResult.IsSuccess);
		Assert.IsInstanceOfType<IContext>(result.Value);
	}
		...
		ITypeReader? reader = null)
	{
		...
		return commandService.ProcessTypeReadersAsync(context, parameter, input, 0);
	}
