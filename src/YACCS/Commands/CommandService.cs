...
public class CommandService : ICommandService
{
		...
		int startIndex)
	{
		...
		{
			return new(CommandScore.InvalidContext(context, command, startIndex));
		}
		else if (input.Length - startIndex < command.MinLength)
		{
			return new(CommandScore.NotEnoughArgs(context, command, startIndex));
		}
		else if (input.Length - startIndex > command.MaxLength)
		{
			return new(CommandScore.TooManyArgs(context, command, startIndex));
		}
		...
		int startIndex)
	{
		...
		{
			return CommandScore.FailedPrecondition(context, command, startIndex, pResult);
		}
		...
		for (var i = 0; i < command.Parameters.Count; ++i)
		{
			...
			if (currentIndex < input.Length || parameter.Length is null)
			{
				...
				{
					return CommandScore.FailedTypeReader(context, command, currentIndex, trResult.InnerResult, parameter);
				}
			...
			{
				return CommandScore.MissingParameterValue(context, command, currentIndex, parameter);
			}

			var ppResult = await command.CanExecuteAsync(parameter, context, value).ConfigureAwait(false);
			if (!ppResult.IsSuccess)
			{
				return CommandScore.FailedParameterPrecondition(context, command, currentIndex, ppResult, parameter);
			}

			args ??= new object?[command.Parameters.Count];
			args[i] = value;
		}
		return CommandScore.CanExecute(context, command, currentIndex, args ?? []);
	}
